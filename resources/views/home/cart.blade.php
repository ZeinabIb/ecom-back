...
<body class="animsition">
	...
	<div class="bg0 p-t-75 p-b-85">
		<div class="container">
					...
					<div class="m-l-25 m-r--38 m-lr-0-xl">
						<div class="wrap-table-shopping-cart">
							<table class="table-shopping-cart">
                                ...
                                @endphp
                                @foreach (Auth::user()->cart->products as $productInCart)
                                <tr class="table_row">
                                    <td class="column-1">
                                        <div class="how-itemcart1">
                                            <img src="/products/{{ $productInCart->image_url }}" alt="IMG">
                                        </div>
                                    </td>
                                    <td class="column-2">{{ $productInCart->name }}</td>
                                    <td class="column-3">$ {{ $productInCart->price }}</td>
                                    <td class="column-4">{{ $productInCart->pivot->quantity }}</td> <!-- Accessing the quantity from pivot table -->
                                    <td class="column-5">$ {{ number_format($productInCart->price * $productInCart->pivot->quantity, 2) }}</td>
                                    <td class="column-6">
                                        {{-- <form method="POST" action="{{ route('user.removeFromCart', ['id'=>$productInCart->id]) }}">
                                            @csrf
                                            @method('DELETE') --}}
                                            <a href="{{ route('user.removeFromCart', ['id'=>$productInCart->id]) }}" class="btn btn-primary text-white">Remove</a>
                                        {{-- </form> --}}
                                    </td>
                                </tr>
                                ...
                                @php
                                    ...
                                    $total += $productInCart->price * $productInCart->pivot->quantity; // Accessing quantity from pivot table
                                @endphp
                                @endforeach
            ...
            <form method="POST" action="{{ route('home.cashOnDeliverySubmit') }}">
              @csrf
                <input type="text" required readonly name="location_lat" id="location_lat_cash" placeholder="Enter your location" class="form-control" style="max-width: 200px;" hidden>
                <input type="text" required readonly name="location_lng" id="location_lng_cash" placeholder="Enter your location" class="form-control" style="max-width: 200px;" hidden>
                <input type="text" required readonly name="totalprice" id="totalprice" value="{{ $total }}" class="form-control" style="max-width: 200px;" hidden>
                <button type="submit" class="flex-c-m stext-101 cl0 size-116 bg3 bor14 hov-btn3 p-lr-15 trans-04 pointer" {{ $total==0?'disabled':'' }}>
                Cash On Delivery 🤑
              </button>
            </form>
            <form method="POST" action="{{ route('home.stripePayement') }}">
                @csrf
                <div class="input-group mb-3">
                    <input type="text" required readonly name="location_lat" id="location_lat_card" placeholder="Enter your location" class="form-control" style="max-width: 200px;" hidden>
                    <input type="text" required readonly name="location_lng" id="location_lng_card" placeholder="Enter your location" class="form-control" style="max-width: 200px;" hidden>
                    <input type="text" required readonly name="totalprice" id="totalprice" value="{{ $total }}" class="form-control" style="max-width: 200px;" hidden>
                </div>
                <button type="submit" class="flex-c-m stext-101 cl0 size-116 bg3 bor14 hov-btn3 p-lr-15 trans-04 pointer mt-7" {{ $total==0?'disabled':''}}>Pay Using Card 💳</button>
            </form>
          </div>
				</div>
			</div>
